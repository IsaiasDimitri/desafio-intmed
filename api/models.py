...
class Medico(models.Model):
    nome = models.CharField("nome", max_length=55, blank=False, null=False)
    crm = models.IntegerField("crm", blank=False, null=False)
    email = models.EmailField("email", max_length=100)
    telefone = models.CharField("telefone", max_length=15)
    especialidade = models.ForeignKey(
        Especialidade, default=None, on_delete=models.SET_DEFAULT
    )
...
class Consulta(models.Model):
    ...
    owner = models.ForeignKey(User, on_delete=models.CASCADE) #paciente
    agenda = models.ForeignKey(Agenda, on_delete=models.CASCADE)
    horario = models.TimeField('horarios', blank=False, null=False)
